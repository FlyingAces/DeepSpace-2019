...
public class ArmSubsystem extends Subsystem {
	...
	public void setMotorSpeed(Angle angle, double speed) {
		...
		case SHOULDER:
			...
			_calculations.setShoulderAngle(speed * .5 + getAngle(Angle.SHOULDER) * MotorSpeeds.SHOULDER_MOTOR_SPEED);
			break;
		case ELBOW:
			//_elbowMotorAndEncoder.set(ControlMode.PercentOutput, speed * MotorSpeeds.ELBOW_MOTOR_SPEED);
			_calculations.setElbowAngle(speed * .5 + getAngle(Angle.ELBOW) * MotorSpeeds.ELBOW_MOTOR_SPEED);
			break;
		case WRIST:
			//_wristMotorAndEncoder.set(ControlMode.PercentOutput, speed * MotorSpeeds.WRIST_MOTOR_SPEED);
			_wristAngle += speed * .5 * MotorSpeeds.WRIST_MOTOR_SPEED;
			break;
		}
	}
